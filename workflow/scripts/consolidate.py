...
def extract_contigs(checkm_df):
    ...
    if binsinfo: df = pd.DataFrame(binsinfo)
    else: df = pd.DataFrame(columns=['binID', 'contig', 'length'])
...
def load_initial_data(checkm_result, contig_data, checkm_data, prefilter):
    ...
    if prefilter:
        ...
        sdf = df
        print ('%i bins total - no prefilter' %(df['ID'].nunique()))
...
def pairwise_process(checkm_result_A, checkm_result_B, contig_data, checkm_data, overlap, rnum):
    ...
    if cIdfA.empty or cIdfB.empty:
        mdf = pd.DataFrame(columns=['binID_A', 'binID_B', 'length_A', 'length_B'])
    else:
...
def process(fnames, outdir, overlap=80, prefilter=False):
    contig_data = {}
    checkm_data = {}
    ...
    for fname in fnames:
        load_initial_data(fname, contig_data, checkm_data, prefilter)
...
if __name__ == '__main__':
    ...
    process(checkmfiles, args.outdir, args.overlap, args.prefilter)